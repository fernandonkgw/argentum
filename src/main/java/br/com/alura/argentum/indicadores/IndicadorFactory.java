...
public class IndicadorFactory {
	...
	public Indicador defineIndicador() {
		...
		if ( nomeIndicadorBase == null || nomeIndicadorBase ==null) {
			return new MediaMovelSimples(new IndicadorFechamento());
		}
		
		try {
			String pacote = "br.com.alura.argentum.indicadores.";
			Class<?> classIndicadorBase = Class.forName(pacote + nomeIndicadorBase);
			Indicador indicadorBase = (Indicador) classIndicadorBase.newInstance();
